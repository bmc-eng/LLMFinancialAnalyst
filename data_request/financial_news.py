...
class FinancialNewsRequester:
    ...
    def build_news_dataset(self, index_members, start_date):
        df_headlines = self._load_all_headlines()
        df_filtered = self._filter_news_headlines(df_headlines, index_members, start_date)
        df_cleaned = self._drop_duplicates(df_filtered)
        pdf = self._sort_headlines(df_cleaned.cache())
        self.spark.stop()
        return pdf
